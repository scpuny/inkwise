...
import { exportMarkdown, copyAsMarkdown, copyAsHtml, exportAsHtml } from "../lib/importExport";
...
export function DocPicker({
  ...
  const handleAddCollection = async () => {
    ...
    if (isTauriEnv()) {
      try { await tryInvoke("create_collection_db", { title: col.title, linkedFolder: null }); } catch {}
    }
  };
  ...
  const handleRename = async (colId: string) => {
    ...
    if (col) {
      ...
      await saveCollections(collections);
      if (isTauriEnv()) {
        try { await tryInvoke("rename_collection_db", { id: colId, title: editTitle.trim() }); } catch {}
      }
    }
  ...
  const handleDeleteCol = async () => {
    ...
    if (idx >= 0) {
      ...
      await saveCollections(collections);
      if (isTauriEnv()) {
        try { await tryInvoke("delete_collection_db", { id: deleteConfirmId }); } catch {}
      }
