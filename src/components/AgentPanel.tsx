...
import { generateArticleReview, saveArticleReview, loadArticleReview, applyOptimization, type ArticleReview } from "../lib/articleReview";
import { loadArticleContent, loadArticleMeta, saveArticleContent } from "../lib/articles";
...
const TABS: { id: TabId; icon: React.ReactNode; label: string }[] = [
  ...
  { id: "review", icon: <Scale size={13} />, label: "评估" },
];
...
function ReviewPanel({ articleId }: { articleId: string | null }) {
  ...
  const handleOptimize = async () => {
    ...
    try {
      ...
      if (optimized && optimized.length > 10) {
        ...
        // Dispatch event so editor picks up the change
