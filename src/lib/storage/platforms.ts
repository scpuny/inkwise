...
export async function getPlatformConfigs(): Promise<PlatformConfig[]> {
  if (isTauriEnv()) {
    try {
      return await invokeOrFallback<PlatformConfig[]>(TauriCommands.GetPlatformConfigs, {}, () => loadPlatformConfigsLocal());
    } catch { /* fallback */ }
  }
...
export async function savePlatformConfig(config: PlatformConfig): Promise<void> {
  if (isTauriEnv()) {
    try {
      await invokeOrFallback(TauriCommands.SavePlatformConfig, { config }, () => {});
      return;
    } catch { /* fallback */ }
  }
...
export async function deletePlatformConfig(id: string): Promise<void> {
  if (isTauriEnv()) {
    try {
      await invokeOrFallback(TauriCommands.DeletePlatformConfig, { id }, () => {});
      return;
    } catch { /* fallback */ }
  }
  const configs = loadPlatformConfigsLocal();
...
export async function verifyPlatformCredentials(platform: string, appId: string, appSecret: string): Promise<boolean> {
  if (isTauriEnv()) {
    try {
      return await invokeOrFallback<boolean>(TauriCommands.VerifyPlatformCredentials, { platform, appId, appSecret }, () => false);
    } catch { /* fallback */ }
  }
...
export async function getPublishHistory(articleId: string): Promise<PublishRecord[]> {
  if (isTauriEnv()) {
    try {
      return await invokeOrFallback<PublishRecord[]>(TauriCommands.GetPublishHistory, { articleId }, () => []);
    } catch { /* fallback */ }
  }
...
export async function addPublishRecord(record: PublishRecord): Promise<void> {
  if (isTauriEnv()) {
    try {
      const records = await invokeOrFallback<PublishRecord[]>(TauriCommands.GetPublishHistory, { articleId: record.articleId }, () => []);
      records.push(record);
      await invokeOrFallback(TauriCommands.SavePublishRecords, { records }, () => {});
      return;
    } catch { /* fallback */ }
  }
