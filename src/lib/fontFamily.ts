// fontFamily.ts — mirrors Reasonix pattern
// data-font-family: "system" | "yahei" | "pingfang" | "noto" | "serif" | "custom"

export const FONT_FAMILIES = ["system", "yahei", "pingfang", "noto", "serif", "custom"] as const;
export type FontFamily = (typeof FONT_FAMILIES)[number];
export const DEFAULT_FONT_FAMILY: FontFamily = "system";

const FONT_FAMILY_KEY = "aiwriter-font-family";
const CUSTOM_FONT_KEY = "aiwriter-font-family-custom";
...
export function setCustomFontName(name: string): void {
  try { localStorage.setItem(CUSTOM_FONT_KEY, name); } catch { /* private mode */ }
}
...
export function applyFontFamily(font: FontFamily): void {
  ...
  try { localStorage.setItem(FONT_FAMILY_KEY, font); } catch { /* private mode */ }
}

export function initFontFamily(): void { applyFontFamily(getFontFamily()); }
