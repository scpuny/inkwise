...
- 直接输出，不要额外说明`, temperature: 0.5, maxTokens: 4096 },
    ...
    builtin("marketing", {
      ...
      scope: "full",
      configs: {
        title: { systemPrompt: `为营销内容生成有吸引力的标题。
...
- 控制在 8-18 字
- 直接输出标题`, temperature: 0.85 },
        description: { systemPrompt: `写一段有吸引力的营销导语。
...
- 20-50 字
- 直接输出`, temperature: 0.8 },
        outline: { systemPrompt: `为营销文案生成结构大纲。
...
- 3-6 个标签，用空格分隔
- 直接输出`, temperature: 0.5 },
        writing: { systemPrompt: `以营销文案风格撰写内容。
...
- 直接输出，不要额外说明`, temperature: 0.75, maxTokens: 4096 },
    ...
    builtin("product-doc", {
      ...
      scope: "full",
      configs: {
        title: { systemPrompt: `为产品文档生成清晰准确的标题。

## 规则
- 直接反映文档内容与用途
- 包含产品名称或功能点
- 控制在 6-18 字
- 直接输出标题`, temperature: 0.3 },
...
- 直接输出，不要额外说明`, temperature: 0.3, maxTokens: 4096 },
    ...
    builtin("review", {
      ...
      scope: "full",
      configs: {
        title: { systemPrompt: `为评论文章生成有吸引力的标题。
...
- 控制在 8-20 字
- 直接输出标题`, temperature: 0.8 },
        description: { systemPrompt: `写一段引人入胜的评论简介。
...
- 20-60 字
- 直接输出`, temperature: 0.75 },
        writing: { systemPrompt: `以评论风格撰写内容。
