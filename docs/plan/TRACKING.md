...
**增量迁移 Batch 1 完成**（2026-07-12）：
- TrashDialog / ProjectPanel / GeneralSection / ProjectExplorer → useCollection hook
- StartupSplash / ProjectFileTree / articleStore → domain 类型导入
- useCollectionCrud 完全重写 → 使用 useCollection 替代旧 storage/collections
- DocPicker → useCollection（renameArticle/trashArticle/loadArticleContent）+ 新 useCollectionCrud
- `storage/collections` 消费者减少 ~5（TrashDialog, ProjectPanel, GeneralSection, ProjectExplorer, useCollectionCrud→DocPicker）
- `storage/articles.ts` 消费者减少 ~1（DocPicker）
- 验证：`tsc --noEmit` 0 errors + `vite build` success
...
**增量迁移 Batch 3 完成**（2026-07-12）：
- SeriesOverview → 完全迁移 useCollection + useDocument（saveSeriesPlan/trashArticle/loadArticleContent + 类型）
- SeriesPlanner → 类型从 storage/collections 改为 domain
- EditorPane → getProvidersSync 从旧 providerModels 改为 useDocument()
- ReviewPanel → 完全替换 loadArticleContent/loadArticleMeta/saveArticleContent 为 useDocument
- ArticleFinalPage → 移除 loadArticleMeta 静态导入（仅动态使用）
...
**增量迁移 Batch 8 完成**（2026-07-12）：**旧存储非桥接消费者全面清零** 🏆
- **migrateArticleDocument 内联**：DocumentStore + TauriDocumentStore + useDocument 新增方法，EditorPane 改用 hook
...
> **周期**：2026-07-12 → 2026-07-12 | **分支**：`main` | **状态**：🟢 已完成
>
...
| 9.4 | **PlanPanel 子拆分** — Sprint 8.2 遗留项 | 架构 | P1 | 🟢 | 07-12 |
| 9.5 | **扫描性能优化** — 增量扫描机制完善（边界情况） | 后端 | P1 | 🟢 | 07-12 |
| 9.6 | **新增 tree-sitter 语言支持** — 安装 Python/Go/Java crate，激活已有 .scm 查询 | 后端 | P1 | 🟢 | 07-12 |
| 9.7 | **激活 root-context 层** — 挂接到 query_execute 调用链，提取函数签名+参数类型 | 后端 | P1 | 🟢 | 07-12 |
| 9.8 | **E2E 测试基础** — Playwright 配置 + 基础用例 | 测试 | P2 | 🟢 | 07-12 |
