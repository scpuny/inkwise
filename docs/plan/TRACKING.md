...
> 最后更新: 2026-07-03 | 分支: `codex/v2.0.0-docs` | 状态: 🟡 规划阶段
...
|---|--------|------|------|--------|--------|------|
| 1.1.1 | `loadCollections` 统一入口，移除直接 `browserLoad` 调用 | `crud.ts`, `CollectionTree.tsx`, `ArticleManager.tsx`, `useCollectionCrud.ts` | 🔴 | — | — | 3 处直接调用需改 |
| 1.1.2 | `saveCollections` 原子写入：Rust JSON 先写，localStorage 后跟，失败则回滚 | `crud.ts` | 🔴 | — | — | 关键一致性的保底 |
...
| # | 功能点 | 文件 | 状态 | 开发者 | 完成日 | 备注 |
|---|--------|------|------|--------|--------|------|
| 1.2.1 | `trashArticle` 补充 SQLite 和向量清理 | `crud.ts`, `lib.rs` | 🔴 | — | — | — |
| 1.2.2 | `removeCollection` 级联删除子文章所有关联数据 | `crud.ts`, `lib.rs` | 🔴 | — | — | content/meta/versions/db/vector |
| 1.2.3 | `deleteSeriesPlan` 补充向量清理 | `series.ts`, `lib.rs` | 🔴 | — | — | series_chunks 清理 |
...
| # | 功能点 | 文件 | 状态 | 开发者 | 完成日 | 备注 |
|---|--------|------|------|--------|--------|------|
| 1.3.1 | `trashArticle` 改为软删除：只移出合集列表，不删 content/meta | `crud.ts` | 🔴 | — | — | 和 1.2.1 联动 |
| 1.3.2 | `permanentlyDeleteArticle` 物理删除 content/meta/versions/db/vector | `crud.ts`, `lib.rs` | 🔴 | — | — | 真正的物理清理 |
| 1.3.3 | `restoreArticle` 改回内容可恢复（从 trash 移回合集） | `crud.ts` | 🔴 | — | — | 恢复后编辑器直接打开 |

### 1.4 风格/动作分离（doc 13 — 核心业务修复）

| # | 功能点 | 文件 | 状态 | 开发者 | 完成日 | 备注 |
|---|--------|------|------|--------|--------|------|
| 1.4.1 | 定义 `WritingStyle` 和 `WritingAction` 类型 | `types.ts` 新文件 | 🔴 | — | — | — |
| 1.4.2 | `ArticleBlueprint.skillId` 改为 `styleId: string`（强必填） | `articleBlueprint.ts` | 🔴 | — | — | 迁移旧数据 |
| 1.4.3 | 迁移前端 WritingSkill → Style（名称不变，结构重构） | `builtins.ts` | 🔴 | — | — | 10 个风格迁移 |
| 1.4.4 | 迁移 Rust builtin_skills 中的动作 → Action 枚举 | `skill.rs` | 🔴 | — | — | 14 个动作迁移 |
...
| 1.4.9 | 审阅结果缓存（文章未修改不重复审阅） | `articleReview.ts` | 🔴 | — | — | doc 17 建议 |
...
| # | 功能点 | 文件 | 状态 | 开发者 | 完成日 | 备注 |
|---|--------|------|------|--------|--------|------|
| 2.1.1 | 定义 `UnifiedSkill` + `ToolCapability` 枚举类型 | `skill.rs`, `types.ts` | 🔴 | — | — | 类型安全替换字符串 |
...
| # | 功能点 | 文件 | 状态 | 开发者 | 完成日 | 备注 |
|---|--------|------|------|--------|--------|------|
| 2.2.1 | `skill.rs` → `skill/` 模块拆分（类型/builtins/store/frontmatter） | `skill/` | 🔴 | — | — | 438 行拆分 |
| 2.2.2 | `agent.rs` → `agent/` 模块拆分（engine/prompt） | `agent/` | 🔴 | — | — | — |
