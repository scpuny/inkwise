...
| 文件 | 行数 | 问题 |
|------|------|------|
| `lib.rs` | 2139 | 80 个命令 + AppState + chat/publish/export 全在一个文件 |
| `store.rs` | 754 | 类型定义 + JSON 读写 + 20+ 结构体 + 30+ CRUD 全混 |
| `db.rs` | 845 | SQLite schema + 7 张表 + FTS5 + migration 耦合 |
| `ai.rs` | 770 | OpenAI + Anthropic + 流式/非流式 + SSE 解析全混 |

**症状**：改一个需求要动 3-4 个不相关的文件，加新提供商要改整段 ai.rs。
...
├── main.rs
│
├── lib.rs (~80 行)              # 只做：AppState + invoke_handler 注册
│
├── commands/                     # 薄命令层，每个文件 ~100 行
│   ├── mod.rs                   #   重新导出
│   ├── document_cmds.rs         #   load/save/delete document
...
│   ├── sqlite.rs                #   SQLite 实现（唯一实现）
│   └── migration.rs             #   schema + 数据迁移
│
├── domain/                       # 纯类型定义
│   ├── mod.rs
...
```rust
/// 统一存储接口 — commands 层不感知具体实现
#[async_trait]
pub trait Storage: Send + Sync {
    async fn load_document(&self, id: &str) -> Result<Option<Document>, StorageError>;
    async fn save_document(&self, doc: &Document) -> Result<(), StorageError>;
    async fn delete_document(&self, id: &str) -> Result<(), StorageError>;
    async fn query_documents(&self, filter: DocumentFilter) -> Result<Vec<Document>, StorageError>;
    async fn list_collections(&self) -> Result<Vec<Collection>, StorageError>;
    async fn save_collection(&self, col: &Collection) -> Result<(), StorageError>;
    // ... 其他操作
}
```

**效果**：命令层永远只调 `Storage` 接口，不关心背后是 SQLite、PostgreSQL 还是文件。

#### 3.7.4 加新提供商的工作量对比

```
现在：改 ai.rs 3 处 + 写新函数 ~100 行 → 改 1 文件，影响现有逻辑
重构后：新建 ai/myprovider.rs → ai/mod.rs 加 1 行 match → 不影响现有逻辑
```
...
```
Sprint 6a（并行）：
  ├── ① domain/ — 从 store.rs 剥离类型定义
  ├── ② storage/ — Storage trait + sqlite.rs + migration.rs
  ├── ③ commands/ — 逐个迁移命令（JSON→SQLite），同步移到 commands/ 下
  ├── ④ ai/ — 从 ai.rs 拆出 openai.rs + anthropic.rs + streaming.rs
  ├── ⑤ 删除旧文件 store.rs / db.rs / 旧 ai.rs 片段
...
| # | 任务 | 产出 | 工作量 |
|---|------|------|--------|
| 6.1 | 定义完整 SQLite schema + Storage trait | `storage/sqlite.rs`, `domain/` | 2d |
| 6.2 | 编写 JSON → SQLite 迁移脚本 | `storage/migration.rs` | 2d |
| 6.3 | 逐个迁移 Tauri 命令到 SQLite，同时移到 `commands/` | `commands/*_cmds.rs`, `lib.rs` 瘦身 | 3d |
| 6.4 | 从 `ai.rs` 拆出 `openai.rs` + `anthropic.rs` + `streaming.rs` | `ai/` 目录 | 1d |
| 6.5 | 重写前端存储层（去掉 localStorage 业务缓存） | `storage/*.ts` | 2d |
| 6.6 | 删除旧文件（store.rs / db.rs / JSON 文件 / 旧前端代码） | 清理 | 0.5d |
| 6.7 | 端到端测试全流程 | 稳定 v3.0-s6 | 2d |

### Sprint 7: 分层拆分（前端 + 后端 Service）

| # | 任务 | 产出 | 工作量 |
|---|------|------|--------|
| 7.1 | Domain 类型独立 + Infrastructure 接口 | `domain/` + 接口定义 | 1d |
| 7.2 | Service 层提取（PlanService / DocumentService） | `service/` 目录 | 3d |
| 7.3 | EditorPane 拆分 + planState 简化 | UI 组件 | 3d |
| 7.4 | 右侧面板合并为 tab 侧栏 | UI 组件 | 1d |

### Sprint 8: 功能增强

| # | 任务 | 产出 | 工作量 |
|---|------|------|--------|
| 8.1 | Skill 纯净分离 + PhaseConfig 独立 | `domain/skill.rs` | 2d |
| 8.2 | 向量搜索 ndarray 矩阵乘加速 | `vector/search.rs` | 1d |
| 8.3 | 一致性交互打磨 | UI 细节 | 2d |
