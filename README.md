...
- **AI 辅助写作** — 续写、改写、润色、翻译、摘要、创意写作，支持流式生成与内联建议；内置 9 种写作技能（通用写作/学术严谨/博客口语/创意文学/自媒体爆款/技术教程/商业文案/新闻报道/营销文案/产品文档/书评影评），支持写作风格快速切换下拉菜单与技能维度进度条可视化；工具调用进度实时展示可折叠卡片
- **多 AI 提供商** — 内置 OpenAI / Anthropic / DeepSeek，支持自定义兼容 API
- **Skill 技能系统** — 可扩展的 Markdown 技能文件，自定义 AI 写作行为
- **文章上下文管理 (ArticleContext)** — 文章级独立样式管理，每篇文章独立持久化样式配置，切换文章自动恢复对应样式
- **专栏规划** — 系列文章规划与管理，分阶段写作流程（初稿→修改→润色→终稿），自动追加序号到标题
- **多平台发布** — 一键发布到微信公众号、今日头条等平台，支持发布历史展开详情、草稿链接、20000 字发布预警、微信错误码中文描述
- **全文检索** — SQLite FTS5 全文搜索，实时检索文章内容
- **项目上下文索引** — 本地目录扫描（5 层深），关联文件夹后主区域展示项目浏览面板，左侧目录树可视化 + 右侧 AI 扫描分析，支持合集切换状态隔离
- **AI 项目结构分析** — 自动扫描项目技术栈、模块划分、核心架构，分析结果 Markdown 渲染为 HTML
